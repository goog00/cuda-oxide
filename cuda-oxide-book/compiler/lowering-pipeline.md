...
LLVM can often rewrite that into `dbg.value` during optimization:

```text
dbg.value -> "this source variable has this SSA value here"
```

Pliron's own optimization passes do not yet perform that debug salvage, so the
full-debug path avoids Pliron `mem2reg` for now. LLVM `-O2` is allowed because
LLVM already has machinery to preserve many variable locations.
